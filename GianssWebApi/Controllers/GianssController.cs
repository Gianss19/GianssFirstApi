...
namespace GianssWebApi.Controllers
{
    ...
    public class GianssController : ControllerBase
    {
        ...
        public GianssController(ILogger<GianssController> logger)
        {
        ...
        {
            return Ok(GianStore.GianList); //Retorna toda la lista de usuarios, con todos sus campos.
        ...
        public ActionResult<GianDto> GetGian(int id)
        {
            ...
            if (id == 0) {
                ...
                return BadRequest("No se ha encontrado"); // se manda un 400 porque no existe el id = 0.

            }
            var gian = GianStore.GianList.FirstOrDefault(x => x.Id == id);//Funcion lambda
        ...
        public ActionResult<GianDto> CreateGian([FromBody] GianDto gianDto)
        {
            ...
            if (GianStore.GianList.FirstOrDefault(x => x.Name.ToLower() == gianDto.Name.ToLower()) != null)
            {
                ModelState.AddModelError("NombreExiste", "Ese nombre ya está registrado!");
        ...
        public IActionResult UpdatePartialGian(int id, JsonPatchDocument<GianDto> patchDto)
        {
            ...
            patchDto.ApplyTo(Gian, ModelState);
